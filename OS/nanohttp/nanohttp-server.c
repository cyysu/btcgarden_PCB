...
//leal, only allow one connections for stability
static int _httpd_max_connections = 1; //20
static int _httpd_timeout = 1;//10;
...
extern pthread_mutex_t network_acc_lock;   //leal add for net acc control
#endif
...
#endif
{
  ...
  while (!done)
  {
    log_verbose2("starting HTTP request on socket(%d)", conn->sock.sock);
...
//  pthread_sigmask(SIG_BLOCK, &thrsigset, NULL);
  pthread_attr_setstacksize(&(conn->attr), 6*1024);
  if ((err =
...
//  sigaddset(&thrsigset, SIGALRM);
  ...
  while (_httpd_run)
  {
	...
	//leal, give up thread, use loop instead
#if 0    
    _httpd_start_thread(conn);
#else
     pthread_mutex_lock(&network_acc_lock);
     httpd_session_main(conn);
